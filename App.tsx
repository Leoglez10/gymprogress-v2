...
const App: React.FC = () => {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case Screen.DASHBOARD: return <Dashboard onStartWorkout={() => navigateTo(Screen.START_WORKOUT)} userAlias={userProfile.alias} goal={userProfile.goal} />;
      ...
      default: return <Dashboard onStartWorkout={() => navigateTo(Screen.START_WORKOUT)} userAlias={userProfile.alias} goal={userProfile.goal} />;
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto relative bg-background-light dark:bg-background-dark shadow-2xl overflow-hidden">
      <div className="flex-1 overflow-y-auto no-scrollbar">
        {renderScreen()}
      </div>
      {showNav && <Navigation currentScreen={currentScreen} onNavigate={navigateTo} />}
    </div>
  );
};
