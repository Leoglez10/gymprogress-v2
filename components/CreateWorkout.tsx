...
const CreateWorkout: React.FC<CreateWorkoutProps> = ({ onBack, onSave, initialRoutineId, userProfile }) => {
  ...
  useEffect(() => {
    ...
    if (initialRoutineId) {
      ...
      if (existing) {
        ...
        setSelectedExercises(existing.exercises.map((ex, idx) => ({ 
          ...ex, 
          uiId: `ui-${ex.exerciseId}-${idx}-${Date.now()}` 
        })));
      }
    }
  }, [initialRoutineId]);
  ...
  const handleSave = () => {
    ...
    const newRoutine: CustomRoutine = {
      id: workoutId || `routine_${Date.now()}`,
      name: workoutName,
      exercises: cleanExercises
    };
    let updatedRoutines = workoutId 
      ? routines.map((r: CustomRoutine) => r.id === workoutId ? newRoutine : r)
      : [newRoutine, ...routines];
  ...
  return (
    <div className="h-full flex flex-col bg-background-light dark:bg-background-dark font-display relative overflow-hidden">
      <div className="flex items-center px-4 py-3 sticky top-0 z-50 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-b border-black/5 dark:border-white/5 pt-[env(safe-area-inset-top)]">
        ...
        <h2 className="text-lg font-bold flex-1 text-center pr-10">Diseñar Rutina</h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-8 pb-44">
        <div className="px-5 pt-6 space-y-3">
          ...
          <input 
            ...
            className="w-full bg-white dark:bg-surface-dark border-0 ring-1 ring-black/5 dark:ring-white/10 rounded-[1.5rem] py-5 px-6 text-xl font-bold placeholder:text-slate-300 focus:ring-2 focus:ring-primary shadow-sm transition-all"
            value={workoutName}
        ...
        <div className="space-y-4">
          ...
          {selectedExercises.length === 0 ? (
            ...
            <>
              <div 
                ...
                className={`flex overflow-x-auto no-scrollbar gap-4 px-6 pb-4 ${draggedItemIndex !== null ? 'snap-none' : 'snap-x snap-mandatory'}`}
              >
                {selectedExercises.map((entry, exIdx) => (
                  <div 
                    ...
                    onDragOver={(e) => onDragOver(e, exIdx)}
                    onDragEnd={onDragEnd}
                    className={`snap-center shrink-0 w-[88vw] bg-white dark:bg-surface-dark rounded-[3rem] p-6 shadow-xl border transition-all duration-300 relative flex flex-col ${
                      draggedItemIndex === exIdx 
                        ? 'border-primary ring-4 ring-primary/10 z-[100] scale-[1.02] shadow-2xl opacity-80 rotate-1' 
                          ...
                          <div className="flex items-center justify-between px-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button 
                                ...
                                className="size-11 flex items-center justify-center rounded-2xl bg-primary text-black transition-all duration-300 active:scale-95 group/dup shadow-lg shadow-primary/20 hover:scale-110 active:rotate-12"
                              >
                                <span className="material-symbols-outlined text-[24px] font-black">content_copy</span>
                              </button>
                              
                              <button 
                                onClick={() => removeSet(exIdx, setIdx)} 
                                title="Eliminar serie"
                                className="size-11 flex items-center justify-center rounded-2xl text-slate-300 hover:text-red-500 hover:bg-red-50 transition-all duration-300 active:scale-90"
                              >
                          ...
                          <div className="space-y-6">
                            <div className="space-y-2">
                               ...
                               <div className="flex items-center justify-between gap-3 bg-white dark:bg-surface-dark p-2 rounded-full shadow-sm border border-black/5 overflow-hidden">
                                  <button onClick={() => updateSet(exIdx, setIdx, 'weight', set.weight - 2.5)} className="size-16 bg-slate-50 dark:bg-background-dark rounded-full flex items-center justify-center text-slate-500 active:bg-primary active:text-black transition-all active:scale-75 shadow-md shrink-0">
                                    <span className="material-symbols-outlined text-3xl font-black">remove</span>
                                  </button>
                                  <input 
                                    type="number" 
                                    min="0"
                                    className="flex-1 bg-transparent border-0 p-0 text-center font-black text-3xl focus:ring-0 tabular-nums min-w-0" 
                                    value={set.weight} 
                            ...
                            <div className="space-y-2">
                               ...
                               <div className="flex items-center justify-between gap-3 bg-white dark:bg-surface-dark p-2 rounded-full shadow-sm border border-black/5 overflow-hidden">
                                  <button onClick={() => updateSet(exIdx, setIdx, 'reps', set.reps - 1)} className="size-16 bg-slate-50 dark:bg-background-dark rounded-full flex items-center justify-center text-slate-500 active:bg-primary active:text-black transition-all active:scale-75 shadow-md shrink-0">
                                    <span className="material-symbols-outlined text-3xl font-black">remove</span>
                                  </button>
                                  <input 
                                    type="number" 
              ...
              <div className="flex justify-center gap-2 pb-4">
                {selectedExercises.map((_, idx) => (
                  <div key={idx} className={`h-1.5 rounded-full transition-all duration-300 ${activeIndex === idx ? 'w-8 bg-primary' : 'w-1.5 bg-slate-200'}`}></div>
                ))}
              </div>
            </>
          )}
          ...
          <div className="px-6">
            <button 
              ...
              className="w-full py-8 rounded-[3rem] border-4 border-dashed border-primary/40 bg-gradient-to-r from-primary/30 to-orange-400/30 text-primary-text font-black flex flex-col items-center justify-center gap-2 active:scale-[0.98] transition-all"
            >
              <span className="material-symbols-outlined text-5xl">add_circle</span>
              <span className="text-[1.2rem] uppercase tracking-[0.1em]">Añadir Ejercicio</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-background-light dark:from-background-dark via-background-light dark:via-background-dark to-transparent z-40">
        <button 
          ...
          className="w-full h-20 bg-primary text-black font-black text-xl rounded-full shadow-2xl active:scale-95 transition-all disabled:opacity-50"
        >
      ...
      {showSaveConfirm && (
        <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-200">
          ...
          <div className="w-full max-w-xs bg-white dark:bg-surface-dark rounded-[3rem] p-8 shadow-2xl flex flex-col items-center text-center gap-6 animate-in zoom-in-95 relative">
            <div className="size-20 rounded-full bg-primary/20 flex items-center justify-center text-primary shadow-lg shadow-primary/20">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium leading-relaxed px-2">¿Estás seguro de que quieres guardar esta rutina en tu biblioteca?</p>
            </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-xl flex items-end animate-in fade-in duration-300">
          ...
          <div className="w-full max-w-md mx-auto bg-white dark:bg-surface-dark rounded-t-[4rem] h-[92vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-500 relative">
            ...
            <div className="p-8 pb-4 flex items-center justify-between">
              ...
              <button onClick={() => { setShowSelector(false); setIsCreatingNew(false); }} className="px-8 py-4 rounded-full bg-primary text-black text-sm font-black transition-all">LISTO</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-32 space-y-4 no-scrollbar">
              {isCreatingNew ? (
                <div className="space-y-10 animate-in slide-in-from-right-4">
                  ...
                  <button onClick={createAndAddExercise} className="w-full py-8 rounded-[2.5rem] bg-black dark:bg-white text-white dark:text-black font-black text-xl shadow-xl transition-all">
                    CREAR Y AÑADIR
                  </button>
                </div>
              ) : (
