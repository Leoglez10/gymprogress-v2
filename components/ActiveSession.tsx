...
const ActiveSession: React.FC<ActiveSessionProps> = ({ routine, onFinish, onCancel, userProfile }) => {
  ...
  useEffect(() => {
    let interval: any;
    if (isResting && restSeconds > 0) { interval = setInterval(() => setRestSeconds(s => s - 1), 1000); }
    else if (restSeconds === 0 && isResting) { setIsResting(false); }
  ...
  const addNewExerciseFromLibrary = (ex: Exercise) => {
    ...
    const newEntry: CustomExerciseEntry = { exerciseId: ex.id, name: ex.name, sets: [{ weight: 0, reps: 10, completed: false }] };
    const finalRoutine = { ...updatedRoutineWithCurrent, exercises: [...updatedRoutineWithCurrent.exercises, newEntry] };
      ...
      {showExerciseSelector && (
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-xl flex items-end animate-in fade-in duration-300">
          ...
          <div className="w-full max-w-md mx-auto bg-white dark:bg-surface-dark rounded-t-[4rem] h-[92vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-500 relative">
            ...
            <div className="p-8 pb-4 flex items-center justify-between"><h3 className="text-3xl font-black tracking-tighter">{isCreatingNew ? 'Nuevo' : 'Biblioteca'}</h3><button onClick={() => setShowExerciseSelector(false)} className="px-8 py-4 rounded-full bg-primary text-black text-sm font-black active:scale-95 transition-all">CERRAR</button></div>
                ...
                <div className="space-y-10 animate-in slide-in-from-right-4">
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {['Push', 'Pull', 'Legs', 'Core'].map(c => (<button key={c} onClick={() => handleNewCategoryChange(c)} className={`px-6 py-4 rounded-2xl text-xs font-black transition-all border-2 ${newExerciseCategory === c ? 'bg-primary border-primary text-black shadow-lg' : 'bg-transparent border-black/5 text-slate-400'}`}>{c}</button>))}
                    </div>
                  </div>
                  <div className="space-y-4">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Músculo Principal</label>
                    <div className="flex flex-wrap gap-3">
                      {CATEGORY_MUSCLE_MAP[newExerciseCategory].map(m => (<button key={m} onClick={() => setNewExerciseMuscle(m)} className={`px-8 py-4 rounded-2xl text-xs font-black transition-all border-2 ${newExerciseMuscle === m ? 'bg-black dark:bg-white text-white dark:text-black border-black dark:border-white shadow-lg' : 'bg-transparent border-black/5 text-slate-400'}`}>{m}</button>))}
                    </div>
                  </div>
                  <button onClick={createAndAddExercise} className="w-full py-8 rounded-[2.5rem] bg-black dark:bg-white text-white dark:text-black font-black text-xl shadow-xl active:scale-95 transition-all">AÑADIR A LA SESIÓN</button>
                </div>
              ) : (
                <>{searchTerm.length > 0 && filteredLibrary.length === 0 && (<button onClick={() => setIsCreatingNew(true)} className="w-full flex items-center gap-6 p-8 rounded-[3rem] bg-primary/10 border-4 border-dashed border-primary/40 text-left active:scale-[0.98] transition-all"><div className="size-20 rounded-[2rem] bg-primary flex items-center justify-center text-black shrink-0"><span className="material-symbols-outlined text-4xl font-black">add</span></div><div className="flex-1"><p className="font-black text-2xl tracking-tight leading-none">Crear "{searchTerm}"</p></div></button>)}{filteredLibrary.map((ex) => (<div key={ex.id} className="w-full flex items-center gap-6 p-5 rounded-[3rem] bg-slate-50 dark:bg-white/5 border border-black/5 shadow-sm active:scale-95 transition-all cursor-pointer" onClick={() => addNewExerciseFromLibrary(ex)}><div className="flex-1 min-w-0"><p className="font-black text-xl leading-tight truncate">{ex.name}</p><p className="text-[11px] text-slate-400 uppercase font-black tracking-widest mt-1">{ex.muscleGroup}</p></div><div className="size-14 rounded-2xl bg-primary flex items-center justify-center text-black shrink-0 shadow-md"><span className="material-symbols-outlined font-black">add</span></div></div>))}</>
              )}
            </div>
          </div>
        </div>
